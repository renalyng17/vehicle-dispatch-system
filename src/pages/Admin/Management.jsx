...
<<<<<<< HEAD
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      try {
        const [vehiclesData, driversData, archivedVehiclesData, archivedDriversData] = await Promise.all([
          api.getVehicles(),
          api.getDrivers(),
          api.getArchivedVehicles(),  // ✅ CORRECT — uses the method you defined
...
<<<<<<< HEAD
  ...
  // DRIVER submit - calls API
  const handleDriverSubmit = async (e) => {
    e.preventDefault();
    setIsSubmittingDriver(true);
    try {
      const normalized = toE164(driverForm.contact);
      const payload = {
...
<<<<<<< HEAD
  ...
  const confirmDeleteAction = async () => {
    if (confirmDelete.type === "vehicle") {
      ...
      try {
        await api.archiveVehicle(vehicleToArchive.id);
        setArchivedVehicles((prev) => [...prev, vehicleToArchive]);
        setVehicles((prev) => prev.filter((_, i) => i !== confirmDelete.idx));
      } catch (err) {
        console.error(err);
      ...
      try {
        await api.archiveDriver(driverToArchive.id);
        setArchivedDrivers((prev) => [...prev, driverToArchive]);
        setDrivers((prev) => prev.filter((_, i) => i !== confirmDelete.idx));
      } catch (err) {
        console.error(err);
...
<<<<<<< HEAD
  ...
  const handleRestore = async (type, idx) => {
    if (type === "vehicle") {
      const v = archivedVehicles[idx];
      try {
        await api.restoreVehicle(v.id);
  ...
  <div className="fixed inset-0 flex items-center justify-center z-50 p-2 sm:p-4">
    <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
      <div className="p-4 sm:p-5">
        ...
        <form className="space-y-3" onSubmit={handleVehicleSubmit}>
          {/* Vehicle Type */}
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Vehicle Type</label>
            <input
              type="text"
              className={`w-full border border-gray-300 rounded-md px-2 py-1.5 text-sm focus:ring-2 focus:ring-green-500 focus:border-green-500`}
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  inputMode="numeric"
                  pattern="[0-9]*"
                  className={`w-full border border-gray-300 rounded-md px-2 py-1.5 text-center text-sm focus:ring-2 focus:ring-green-500 focus:border-green-500`}
          ...
          <div className="relative">
            ...
            <button
              type="button"
              className={`w-full border ${showFuelTypeDropdown ? 'border-green-500 ring-2 ring-green-200' : 'border-gray-300'} rounded-md px-2 py-1.5 text-sm flex items-center justify-between hover:bg-gray-50`}
              onClick={(e) => {
                e.stopPropagation();
              ...
              >
                {["BIO-DIESEL", "DIESEL", "KEROSENE"].map((type) => (
                  <button
                    key={type}
                    type="button"
                    className={`w-full text-left px-2 py-1.5 hover:bg-gray-100 text-sm ${vehicleForm.fuelType === type ? "bg-gray-100 font-medium" : ""}`}
                    onClick={() => {
          ...
          {/* Fleet Card Dropdown */}
          <div className="relative">
            <label className="block text-xs font-medium text-gray-600 mb-1">Fleet Card</label>
            <button
              type="button"
              className={`w-full border ${showFleetCardDropdown ? 'border-green-500 ring-2 ring-green-200' : 'border-gray-300'} rounded-md px-2 py-1.5 text-sm flex items-center justify-between hover:bg-gray-50`}
              onClick={(e) => {
                e.stopPropagation();
              ...
              >
                {["Available", "Unavailable"].map((status) => (
                  <button
                    key={status}
                    type="button"
                    className={`w-full text-left px-2 py-1.5 hover:bg-gray-100 text-sm ${vehicleForm.fleetCard === status ? "bg-gray-100 font-medium" : ""}`}
                    onClick={() => {
          ...
          {/* RFID Dropdown */}
          <div className="relative">
            <label className="block text-xs font-medium text-gray-600 mb-1">RFID</label>
            <button
              type="button"
              className={`w-full border ${showRfidDropdown ? 'border-green-500 ring-2 ring-green-200' : 'border-gray-300'} rounded-md px-2 py-1.5 text-sm flex items-center justify-between hover:bg-gray-50`}
              onClick={(e) => {
                e.stopPropagation();
              ...
              >
                {["Available", "Unavailable"].map((status) => (
                  <button
                    key={status}
                    type="button"
                    className={`w-full text-left px-2 py-1.5 hover:bg-gray-100 text-sm ${vehicleForm.rfid === status ? "bg-gray-100 font-medium" : ""}`}
                    onClick={() => {
          ...
          <div className="flex justify-end gap-2 pt-3">
            ...
            <button
              type="submit"
              className={`px-3 py-1.5 bg-green-600 rounded-md text-sm font-medium text-white hover:bg-green-700 transition-colors ${(!isVehicleFormValid() || isSubmittingVehicle) ? "opacity-50 cursor-not-allowed" : ""}`}
              ...
              <div className="p-6">
                ...
                <form className="space-y-4" onSubmit={handleDriverSubmit}>
                  ...
                  <div className="flex justify-end gap-3 pt-4">
                    ...
                    <button
                      type="submit"
                      className={`px-4 py-2 bg-green-600 rounded-lg font-medium text-white hover:bg-green-700 transition-colors ${(!isDriverFormValid() || isSubmittingDriver) ? "opacity-50 cursor-not-allowed" : ""}`}
